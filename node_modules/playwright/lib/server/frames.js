...
class Frame extends events_1.EventEmitter {
    ...
    async selectOption(selector, elements, values, options = {}) {
    ...
    async _waitForFunctionExpression(expression, isFunction, arg, options = {}) {
        ...
        return this._page._runAbortableTask(progress => this._scheduleRerunnableHandleTask(progress, 'main', task), this._page._timeoutSettings.timeout(options));
    }
    async title() {
        const context = await this._utilityContext();
...
async function runNavigationTask(frame, options, task) {
    const page = frame._page;
    const controller = new progress_1.ProgressController(page._timeoutSettings.navigationTimeout(options));
