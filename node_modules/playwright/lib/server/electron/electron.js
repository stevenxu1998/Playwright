...
class ElectronApplication extends events_1.EventEmitter {
    ...
    async _waitForEvent(event, predicate) {
        const progressController = new progress_1.ProgressController(this._timeoutSettings.timeout({}));
        if (event !== ElectronApplication.Events.Close)
            this._browserContext._closePromise.then(error => progressController.abort(error));
        return progressController.run(progress => helper_1.helper.waitForEvent(progress, this, event, predicate).promise);
    }
    async _init() {
...
class Electron {
    async launch(executablePath, options = {}) {
        ...
        return progress_1.runAbortableTask(async (progress) => {
            ...
            await app._init();
            return app;
        }, timeoutSettings_1.TimeoutSettings.timeout(options), 'browser');
    }
}
